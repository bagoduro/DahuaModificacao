import copy
import hashlib  # <--- Adicione esta linha para gerar os hashes MD5
...
class DahuaFunctions(Network):
    ...
    def user_add(self, msg):
            """ 
            Cria um novo usuário administrativo calculando o hash MD5 
            correto exigido pelo backend do firmware.
            Uso no console: usermgr_fix add <usuario> <senha_texto_claro>
            """
            cmd = msg.split()
            
            # Validação básica de argumentos (espera: comando usuario senha)
            if len(cmd) < 3:
                log.info("Uso: usermgr_fix add <usuario> <senha>")
                return False
    
            username = cmd[1]
            password_plain = cmd[2]
    
            # 1. Gera o MD5 em letras maiúsculas estrito (padrão aceito na criação via RPC)
            md5_simple = hashlib.md5(password_plain.encode('utf-8')).hexdigest().upper()
    
            log.info(f"Convertendo senha '{password_plain}' para Hash MD5: {md5_simple}")
    
            # Monta a query RPC com a AuthorityList administrativa completa exigida pelo firmware
            query_args = {
                ...
                "params": {
                    "user": {
                        ...
                        "Reserved": False,
                        "Type": "Normal",
                        "Memo": "Added via DahuaConsole Fix",
                        "MacOnly": "",
                        "MaxMonitorChannels": 0,
                        "AuthorityList": [
                            "AuthUserMag", "Monitor_01", "Replay_01", "AuthSysCfg", 
                            "AuthSysInfo", "AuthManuCtr", "AuthBackup", "AuthStoreCfg", 
                            "AuthEventCfg", "AuthNetCfg", "AuthPeripheral", "AuthAVParam"
                        ]
                    }
                },
                "id": self.ID
            ...
            # Envia a chamada utilizando o método interno do próprio script
            dh_data = self.send_call(query_args)
            
            if dh_data and dh_data.get('result'):
                log.success(f"Usuário '{username}' criado com sucesso no grupo admin!")
                return True
            else:
                log.failure(f"Falha ao criar usuário: {dh_data}")
                return False
    ...
    def user_manager(self, msg):
        """
        Gerencia usuários: listar, ver capacidades, adicionar, deletar.
        Uso:
            usermgr list
            usermgr caps
            usermgr add <nome> <senha>
            usermgr del <nome>
        """
        cmd = msg.split()
        usage = {
            "list": "Lista todos os usuários",
            "caps": "Mostra capacidades de gerenciamento",
            "add": "Adiciona um novo usuário (admin). Ex: usermgr add jose 123456",
            "del": "Remove um usuário. Ex: usermgr del jose"
        }

        if len(cmd) == 1 or (len(cmd) == 2 and cmd[1] == '-h'):
            log.info("Comandos usermgr:")
            for key, help_text in usage.items():
                log.success(f"  {key}: {help_text}")
            return

        subcmd = cmd[1].lower()

        if subcmd == "list":
            query = {"method": "userManager.getUserInfoAll", "params": {}}
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                users = resp.get("params", {}).get("users", [])
                log.info(f"Total de usuários: {len(users)}")
                for u in users:
                    log.success(f"  {u['Name']} (grupo: {u['Group']}, id: {u['Id']}) - {u.get('Memo', '')}")
            else:
                log.failure("Falha ao listar usuários")
            return

        if subcmd == "caps":
            query = {"method": "userManager.getCaps", "params": {}}
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                log.info("Capacidades do gerenciador de usuários:")
                print(json.dumps(resp.get("params", {}), indent=2))
            else:
                log.failure("Falha ao obter capacidades")
            return

        if subcmd == "add":
            if len(cmd) < 4:
                log.failure("Uso incorreto. Ex: usermgr add <nome> <senha>")
                return
            username = cmd[2]
            password = cmd[3]

            # 1. Consulta dinamicamente a lista de permissões suportada pelo firmware atual
            query_auth = {
                "method": "userManager.getAuthorityList",
                "params": {}
            }
            log.info("Consultando dicionário de permissões do dispositivo...")
            resp_auth = self.send_call(query_auth, errorcodes=True)
            
            if resp_auth and resp_auth.get("result"):
                authority_list = resp_auth.get("params")
                log.success(f"Dicionário obtido com sucesso ({len(authority_list)} permissões localizadas).")
            else:
                log.warning("Falha ao obter lista dinâmica. Adotando fallback estruturado.")
                authority_list = ["Monitor", "Monitor_01", "Monitor_02", "Replay", "AuthUserMag", "AuthSysCfg", "AuthNetCfg"]

            # 2. Resgata a função nativa de hash do ecossistema pwdmanager
            from pwdmanager import dahua_gen2_md5_hash
            
            # 2. Define o Realm real extraído do hardware em laboratório
            realm_atual = self.dh_realm
            
            log.info(f"Gerando assinatura Gen2 baseada no Realm dinâmico: {realm_atual}")
            
            from pwdmanager import dahua_gen2_md5_hash
            dh_hash = dahua_gen2_md5_hash(
                dh_realm=realm_atual, 
                username=username, 
                password=password, 
                return_hash=True
            )

            # 3. Monta a requisição estruturada de criação conforme o mantenedor
            query = {
                "method": "userManager.addUser",
                "params": {
                    "user": {
            ...
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                log.success(f"[+] Usuário '{username}' criado com sucesso!")
            else:
                log.failure(f"[-] Falha ao criar usuário: {resp}")
            return

        if subcmd == "del":
            if len(cmd) < 3:
                log.failure("Uso incorreto. Ex: usermgr del <nome>")
                return
            username = cmd[2]
            query = {
            ...
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                log.success(f"[+] Usuário '{username}' removido com sucesso!")
            else:
                log.failure("[-] Falha ao remover usuário.")
            return
        ...
        if subcmd == "add":
            if len(cmd) < 4:
                log.failure("Uso: usermgr add <nome> <senha>")
                return
            username = cmd[2]
            password = cmd[3]
            # Calcula MD5 da senha (conforme exigido pelo dispositivo)
            import hashlib
            password_hash = hashlib.md5(password.encode()).hexdigest().upper()
            auth_list = [
                "AuthUserMag", "AuthSysCfg", "AuthSysInfo", "AuthManuCtr",
                "AuthBackup", "AuthStoreCfg", "AuthEventCfg", "AuthNetCfg",
                "AuthRmtDevice", "AuthSecurity", "AuthMaintence",
                "Monitor_01", "Monitor_02", "Monitor_03", "Monitor_04",
                "Monitor_05", "Monitor_06", "Monitor_07", "Monitor_08",
                "Replay_01", "Replay_02", "Replay_03", "Replay_04",
            ...
            params = {
                "user": {
                    ...
                    "Reserved": False,
                    "Type": "Normal",
                    "Memo": "Added by DahuaConsole",
                    "MacOnly": "",
                    "MaxMonitorChannels": -1,
                    "PasswordModifiedTime": "2000:00:00-00:00:00"
                }
            }
            # Verifica se usuário já existe
            query_check = {"method": "userManager.getUserInfoAll", "params": {}}
            resp_check = self.send_call(query_check, errorcodes=True)
            if resp_check and resp_check.get("result"):
                users = resp_check.get("params", {}).get("users", [])
                for u in users:
                    if u.get("Name") == username:
                        log.failure(f"Usuário '{username}' já existe!")
                        return
            query = {"method": "userManager.addUser", "params": params}
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                log.success(f"Usuário '{username}' criado com sucesso!")
            else:
        ...
        if subcmd == "del":
            if len(cmd) < 3:
                log.failure("Uso: usermgr del <nome>")
                return
            username = cmd[2]
            query = {"method": "userManager.deleteUser", "params": {"name": username}}
            resp = self.send_call(query, errorcodes=True)
            if resp and resp.get("result"):
                log.success(f"Usuário '{username}' deletado com sucesso!")
            else:
                log.failure(f"Falha ao deletar usuário: {resp}")
            return

        log.failure(f"Subcomando desconhecido: {subcmd}. Use 'usermgr -h' para ajuda.")
